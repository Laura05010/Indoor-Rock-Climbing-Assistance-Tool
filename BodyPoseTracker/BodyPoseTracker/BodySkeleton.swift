...
//  BodyPoseTracker
//
//  Created by Lucas Noritomi-Hartwig on 2023-09-22.
//
...
class BodySkeleton: Entity {
    ...
    required init(for bodyAnchor: ARBodyAnchor) {
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            ...
            case "neck_1_joint", "neck_2_joint", "neck_3_joint", "neck_4_joint", "head_joint", "left_shoulder_1_joint", "right_shoulder_1_joint":
                jointRadius *= 0.5
            case "jaw_joint", "chin_joint", "left_eye_joint", "left_eyeLowerLid_joint", "left_eyeUpperlid_joint", "left_eyeball_joint", "nose_joint", "right_eye_joint", "right_eyeLowerLid_joint", "right_eyeUpperLid _joint", "right_eyeball_joint":
                jointRadius *= 0.2
                jointColor = .yellow
        ...
        for bone in Bones.allCases {
            ...
            let bonentity = createBoneEntity(for: skeletonBone)
            bones [bone .name] = bonentity
            self.addChild (bonentity)
        }
    }
    ...
    func update(with bodyAnchor: ARBodyAnchor) {
        ...
        for jointName in ARSkeletonDefinition.defaultBody3D.jointNames {
            if let jointEntity = joints[ jointName],
               let jointEntityTransform = bodyAnchor.skeleton.modelTransform(for: ARSkeleton.JointName (rawValue:
                                                                                                            jointName)) {
    ...
    // Constructs a sphere entity for each joint in a skeleton
    ...
    // Constructs a bone (a bone may not exist in a joint)
    private func createSkeletonBone(bone: Bones, bodyAnchor: ARBodyAnchor) -> SkeletonBone? {
        ...
        let jointFrontEntityOffsetFromRoot = simd_make_float3(fromJointEntityTransform.columns.3) // relative to root (i.e. hipjoint)
        let jointFromEntityPosition = jointFrontEntityOffsetFromRoot + rootPosition // relative to workd reference frame
